...
struct
  ...
  let query man q = S.query (coerce_man man) q

  let assign man lv e = S.assign (coerce_man man) lv e
  let vdecl man v = S.vdecl (coerce_man man) v
  let branch man e b = S.branch (coerce_man man) e b
  let body man fd = S.body (coerce_man man) fd
  let return man r fd = S.return (coerce_man man) r fd
  let asm man = S.asm (coerce_man man)
  let skip man = S.skip (coerce_man man)
  let special man lv f args = S.special (coerce_man man) lv f args
  let enter man lv f args = S.enter (coerce_man man) lv f args
  let event man ev man2 = S.event (coerce_man man) ev (coerce_man man2)

  let combine_env man lv e f args c d ask =
    S.combine_env (coerce_man man) lv e f args (Obj.magic c) d ask
  let combine_assign man lv e f args c d ask =
    S.combine_assign (coerce_man man) lv e f args (Obj.magic c) d ask

  let paths_as_set man = S.paths_as_set (coerce_man man)
  let threadenter man ~multiple lv f args = S.threadenter (coerce_man man) ~multiple lv f args
  let threadspawn man ~multiple lv f args fman =
    S.threadspawn (coerce_man man) ~multiple lv f args (coerce_man fman)
end
...
module AnalyzeCFG_2 (Cfg:CfgBidirSkip) (Spec:Spec) (Inc:Increment) =
struct
  ...
  struct
    ...
    (* The Equation system *)
    module EQSys = Constraints_wp.FromSpec (Spec) (Cfg)
  ...
  let analyze (file: file) (startfuns, exitfuns, otherfuns: Analyses.fundecs) =
    ...
    let solve_and_postprocess () =
      let lh, gh =
        ...
        (*### START OF LOG ###*)
        (*print set of entrystates, entrystatex_global and startvars'*)
    ...
    let log_lh_contents lh =
      ...
      LHT.iter (fun (node, ctx) state ->
          ...
          (* Logs.debug "  State is_top: %b" (Spec.D.is_top state);
             Logs.debug "  State is_bot: %b" (Spec.D.is_bot state); *)

          (* Test state pretty printing with exception handling *)
          (try
             Logs.debug "  State: %a" Spec.D.pretty state
           with e ->
...
module AnalyzeCFG_3 (Cfg:CfgBidirSkip) (Spec_forw:Spec) (Spec_backw: Spec with type C.t = Spec_forw.C.t ) (Inc:Increment) =
...
struct
  ...
  let analyze (file: file) (startfuns, exitfuns, otherfuns: Analyses.fundecs) =
    ...
    (* muss iwie die typen exposen..?*)

    (* Doing forwards and backwards inits*)
    (* Simulate globals before analysis. *)
    let gh = GHT.create 13 in
    let getg v = GHT.find_default gh v (EQSys.G.bot ()) in
    ...
    let do_forwards_inits () = 

      (* analyze cil's global-inits function to get a starting state *)
      let do_global_inits (file: file) : Spec_forw.D.t * fundec list =
        let man =
          { ask     = (fun (type a) (q: a Queries.t) -> Queries.Result.top q)
          ; emit   = (fun _ -> failwith "Cannot \"emit\" in global initializer context.")
          ; node    = MyCFG.dummy_node
        ...
        let transfer_func (st : Spec_forw.D.t) (loc, edge) : Spec.D.t =
          match edge with
          | MyCFG.Entry func        -> Spec.body {man with local = st} func
          | MyCFG.Assign (lval,exp) ->
            begin match lval, exp with
              | (Var v,o), (AddrOf (Var f,NoOffset))
                when v.vstorage <> Static && isFunctionType f.vtype ->
      ...
      let startstate, _ = do_global_inits file

      in
      startstate
    in

    let do_backwards_inits () = () in

    let calculate_solver_input () = () in
    ...
    let solve_and_postprocess () =
      ...
      (* check for dead code at the last state: *)
      let main_sol = try LHT.find lh (List.hd startvars') with Not_found -> EQSys.D.bot () in
      if EQSys.D.is_bot main_sol then
        M.warn_noloc ~category:Deadcode "Function 'main' does not return";
...
(** Given a [Cfg] and a [Spec], and unused [Inc] computes the solution to [???] *)
module AnalyzeCFG_WP (Cfg:CfgBidirSkip) (Spec:Spec) (Inc:Increment) =
struct
  ...
  (** The main function to preform the selected analyses. *)
  let analyze (file: file) (startfuns, exitfuns, otherfuns: Analyses.fundecs) =
    Messages.warn "Starting analysis '%s:'" (Spec.name ());    

    Logs.debug "Spec: Type of D: %s" (Spec.D.name ());
    Logs.debug "Spec: Type of G: %s" (Spec.G.name ());

    Logs.debug "Startfuns: %s" (List.fold_left (fun a f -> a ^ " ; "  ^ f.svar.vname) "" startfuns);
    ...
    (* empty entrystates:*)
    (* let entrystates = [] in
       let entrystates_global = [] in
       let startvars' = [] in *)

    (* Non-Empty entrystates copied*)
    let man e =
      { ask     = (fun (type a) (q: a Queries.t) -> Queries.Result.top q)
      ; emit   = (fun _ -> failwith "Cannot \"emit\" in enter_with context.")
      ; node    = MyCFG.dummy_node
    ...
    (*what if i use exitwars as starvars?  *)

    let (local_res, global_res), _ = Slvr.solve entrystates entrystates_global startvars' None in
    let local_xml = solver2source_result local_res in
    ...
    ResultOutput.output (lazy local_xml) (fun _ -> true) global_res make_global_fast_xml (module FileCfg);
    ();
